...
### 🛡️ 安全与防御
- 安全基准测试：跑公开测试集（Gandalf / HackAPrompt）发布拦截率数据
- 突出展示 SkillHub 第三方安全检测报告（科恩实验室 + 云鼎实验室）
- 注入变异模拟器（侦测盲点，记录不自动生成规则，防误报）

### 🧠 记忆与检索
- 自适应检索权重：利用用户弱反馈信号动态调整 BM25/语义/时间权重
- `moyu doctor` 命令：一键记忆健康检查（冗余检测、过期关系、断裂引用、安全日志摘要）
- `moyu snapshot` 命令：导出只读记忆快照文件
- `moyu demo-attack` 命令：交互式模拟注入攻击演示防御链

### 📝 文档与体验
- README 能力列表改为层内独立编号，章节标题不写总数
- 安全能力评估表数据更新（双层测试 74.8% + 词库 513 条）
- API 独立参考文档 + 架构图
- 提供 3 个 10 行集成示例（examples/ 目录）

### 🏗️ 架构
- Prompt 即记忆：安检规则改为可自更新 prompt 集合（等待 v2.5.2 发版后启动）
- 可选 LanceDB 后端、GGUF 本地嵌入模型（暂缓，不做主要规划）
